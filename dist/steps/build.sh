...
LINE_TO_WRITE=" 1: $KEYWORD;$LINE_TO_CHECK"
...
if  grep -q "$STRING" "$PROJECT_SETTINGS_PATH" ; then
  sed 's/ARTEMIS/'"$SCRIPTINGDEFINESYMBOL"'/g' $PROJECT_SETTINGS_PATH
  sed -i 's/ARTEMIS/'"$SCRIPTINGDEFINESYMBOL"'/g' $PROJECT_SETTINGS_PATH
else
  sed '1: UNITY_POST_PROCESSING_STACK_V2'"1: UNITY_POST_PROCESSING_STACK_V2;$SCRIPTINGDEFINESYMBOL"'/g' $PROJECT_SETTINGS_PATH
